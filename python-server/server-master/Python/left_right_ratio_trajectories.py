...
# Function that generates an animate plot for the left fence
...
# Function that generates an animate plot for the right fence
...
# Function that generates an animate plot for the ratio between left and right 
# numerical/non-numerical variable
...
# This function represents the generation of a new combination of parameters 
# for the left fence, checking its validity.
...
# This function is the same as GenerateNewTrialLeft, but it generates a combination
# of parameters for the right fence.        
...
# This function computes the ration between the left and the right numerical/non-numerical
# variable --> computes the RATIO of Number of Chickens, Item Surface Area and Field Area.
# The value computed is appended to a specific array, thanks to which we can plot a 3D
# graph to show the difficulty of each point.    
def CalculateRatio():
    i = 0
    for i in range(len(fa_array_left)):   
        temp_fa = np.log2(fa_array_left[i] / fa_array_right[i])
        temp_isa = np.log2(isa_array_left[i] / isa_array_right[i])
        if(num_array_left[i] != 0 and num_array_right[i] != 0):
            temp_num = np.log2(num_array_left[i] / num_array_right[i])
        else :
            temp_num = 0           
...
# This function plots a two-dimensional graph which is extrapolated from the 
# previous three-dimensional graph computed by the function CalculateRatio  
def PlotPlane():
